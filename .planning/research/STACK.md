...
**Domain:** Browser-only MIDI piano practice tool — MusicXML notation rendering + mistake annotation
**Researched:** 2026-09-13
**Confidence:** MEDIUM-HIGH (library choice and MIDI/Audio timing semantics HIGH; specific coloring/cursor API details MEDIUM — verify against the installed version's TypeScript defs before building on them)
...
No bundler and no `package.json`-driven build are required to **run** the app (per the constraint). Two supported paths:
...
npm install opensheetmusicdisplay@2.1.2 idb@8.0.3
...
If local `node_modules` copies are used instead of CDN imports, serve them as ES modules with **bare import rewriting** (e.g. a two-line dev-only import map in the HTML pointing bare specifiers at `./node_modules/...`) rather than a bundler — this keeps "open the file / run one static server" intact while still letting you `npm install` for reproducible versions and offline work.
...
**If the analysis engine (alignment scoring, aggregation) grows complex enough to want real unit-test coverage in isolation from OSMD/DOM:**
...
**If you later add the "phone/tablet on the music stand" variant (explicitly out of scope now but noted as deferred):**
...
---
*Stack research for: browser-based MIDI piano practice tool (MusicXML rendering + mistake annotation)*
*Researched: 2026-09-13*
