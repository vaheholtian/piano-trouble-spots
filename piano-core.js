...
globalThis.PianoCore = (() => {
  ...
  function parseMidi(buffer) {
    ...
    if (headerLength < 6 || format > 1 || !nTracks || !division || (division & 0x8000)) {
    ...
    for (let track = 0; track < nTracks; track++) {
      ...
      while (p < end) {
        ...
        if (kind === 9 && second > 0) {
          ...
          open.get(key).push({pitch: first, tick, track, channel});
        } else if (kind === 8 || (kind === 9 && second === 0)) {
          const note = open.get(key)?.shift();
          if (note && channel !== 9) raw.push({...note, endTick: tick});
        }
      }
      if ([...open.values()].some(queue => queue.some(n => n.channel !== 9))) {
  ...
  function validateSong(song) {
    ...
    for (const n of song.notes) {
      if (!n || !integer(n.pitch, 0, 127) || !finite(n.on, 0, 86400) || !finite(n.off, n.on, 86400) ||
          !integer(n.bar, 1, song.nBars) || !finite(n.beat, 1, 256) || !integer(n.track, 0, 65535) || n.on < previous) {
  ...
  function sectionNotes(song, settings) {
    ...
    const notes = song.notes.map((n, id) => ({...n, id})).filter(n => n.bar >= settings.startBar && n.bar <= endBar &&
      (settings.track === 'all' || n.track === settings.track));
  ...
  function analyze(song, settings, performed) {
    ...
    const anchor = first.on, used = new Set(), observations = [];
    for (const r of reference) {
      const siblings = byPitch.get(r.pitch), index = siblings.indexOf(r), before = siblings[index - 1], after = siblings[index + 1];
      ...
      const candidates = performed.map((n, id) => ({n, id, time: n.on - anchor})).filter(q =>
        q.n.pitch === r.pitch && !used.has(q.id) && q.time > left && q.time < right);
      // Multiple attacks in one window are ambiguous: do not invent a duration measurement.
      if (candidates.length !== 1) continue;
      const q = candidates[0]; used.add(q.id);
  ...
  function readSession(text) {
    ...
    if (!Array.isArray(data.takes) || data.takes.length > 2000) throw new Error('Invalid session takes.');
    const takes = data.takes.map(t => {
      if (!t || typeof t.at !== 'string' || !Number.isFinite(Date.parse(t.at)) || !Array.isArray(t.notes) || !t.notes.length || t.notes.length > 20000) {
        throw new Error('Invalid recorded take.');
      }
      validateSettings(t.settings, data.song);
      let previous = -1;
      for (const n of t.notes) {
        if (!n || !integer(n.pitch, 0, 127) || !integer(n.channel, 0, 15) || !finite(n.on, 0, 86400) ||
      ...
      return {at: t.at, settings: t.settings, notes: t.notes, analysis: analyze(data.song, t.settings, t.notes)};
    });
    return {song: data.song, settings: data.settings, takes};
  }

  return {parseMidi, validateSong, validateSettings, sectionNotes, analyze, aggregate, readSession, pitchName, median};
})();
