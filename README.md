...
Tests parse the verification-ladder MusicXML files through the real OpenSheetMusicDisplay 2.1.2 parser (under jsdom, no rendering).

## Developer checks

- `npm run check` — shell-neutral static gates for the `file://` run path: no ES-module script tags, the pinned OSMD CDN URL, dependency-ordered local scripts, no ES-module syntax in `src/`.
...
Both are plain `node` scripts and run identically from PowerShell, cmd, or Git Bash.
